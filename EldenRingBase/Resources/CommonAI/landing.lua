...
function LiftOff_Update(arg0, arg1)
    ...
    if (local2 == GOAL_RESULT_Success or local2 == GOAL_RESULT_Failed) and (not not arg0:IsLand() or local1 < local0) then
        arg1:AddSubGoal(GOAL_COMMON_Attack, 10, 9520, TARGET_NONE, DIST_None)
    end
    if not arg0:IsLand() and local0 <= local1 then
...
function Landing_Landing_Update(arg0, arg1)
    ...
    if (local0 == GOAL_RESULT_Success or local0 == GOAL_RESULT_Failed) and not arg0:IsLand() then
        arg1:AddSubGoal(GOAL_COMMON_Attack, arg1:GetLife(), 9510, TARGET_NONE, DIST_None)
    end
