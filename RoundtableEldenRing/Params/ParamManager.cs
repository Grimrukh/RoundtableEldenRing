...
public class ParamManager : GameMonitor
{
    ...
    Dictionary<ParamType, MemoryPARAM> ParamsInMemory { get; } = new();
    ...
    /// <summary>
    /// Retrieve found `MemoryPARAM` or try to load it now (first time).
    /// 
    ...
    public MemoryPARAM? GetParam(ParamType paramType)
    {
        if (ParamsInMemory.TryGetValue(paramType, out MemoryPARAM? param))
        ...
        ParamsInMemory[paramType] = new MemoryPARAM(paramPointer, ParamdefsByType[paramdefType]);
        Logging.Info($"IsGameLoaded PARAM {Enum.GetName(paramType)} in memory ({ParamsInMemory[paramType].Rows.Count} rows).");
    ...
    ///
    /// These requests shouldn't come much, because we favor `MemoryPARAM.FastSet()` now.
    /// </summary>
    /// <returns></returns>
    protected override bool OnUpdate(long updateTime, long gameLoadedTime)
    {
        ...
        {
            MemoryPARAM? param = GetParam(paramType);
